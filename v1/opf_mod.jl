...
function post_opf_mod(pm::PM.GenericPowerModel)
	...
	@variable(pm.model, 0.8*pm.ref[:bus][i]["pd"] <= pd[i in keys(pm.ref[:bus])] <= pm.ref[:bus][i]["pd"])
    @variable(pm.model, 0.8*pm.ref[:bus][i]["qd"] <= qd[i in keys(pm.ref[:bus])] <= pm.ref[:bus][i]["qd"])

    v = getindex(pm.model, :v)
    p = getindex(pm.model, :p)  # line flow variables
    pg = getindex(pm.model, :pg)    # active generation variables
    qg = getindex(pm.model, :qg)


if pos == 0
    @objective(pm.model, Max, p[line] - (
    sum(l_pd_val[i]*pd[i] + l_qd_val[i]*qd[i] for i in keys(pm.ref[:bus])) + sum(l_pg_val[i]*pg[i] + l_qg_val[i]*qg[i] for i in keys(pm.ref[:gen]))    )
    )
end
...
if pos == 1
    ...
    # standard AC-OPF objective
    # PM.objective_min_fuel_cost(pm)
