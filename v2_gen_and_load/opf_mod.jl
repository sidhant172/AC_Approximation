...
function post_opf_mod(pm::PM.GenericPowerModel)
	...
	@variable(pm.model, 0.8*pm.ref[:bus][i]["pd"] <= pd[i in keys(pm.ref[:bus])] <= 1.2*pm.ref[:bus][i]["pd"])
    @variable(pm.model, 0.8*pm.ref[:bus][i]["qd"] <= qd[i in keys(pm.ref[:bus])] <= 1.2*pm.ref[:bus][i]["qd"])
