...
for inflation in inflation_factors
    ctr = ctr+1
    vars = matread("../resultsJacobian/case"string(num_bus)"/jacobian_error_real_"string(inflation)".mat")
    p_error_jac[:,ctr] = vars["err"]
    vars = matread("../resultsJacobian/case"string(num_bus)"/jacobian_error_reactive_"string(inflation)".mat")
...
for i = 1:num_branch
    ...
    savefig(perr_plot, "plotsJacobian/case"string(num_bus)"/approximation_error_comparison/approx_error_real_line_"string(i)".pdf")
    ...
    savefig(qerr_plot, "plotsJacobian/case"string(num_bus)"/approximation_error_comparison/approx_error_reactive_line_"string(i)".pdf")
