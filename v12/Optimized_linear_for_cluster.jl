...
@show algorithm = convert(Int64,parse(Float64,ARGS[6]))
...
elseif algorithm == 2
    ...
    cnst_gen_max_iter  = 5  # max iterations for constraint generation
end
...
if quantity_to_approx == "line_real_power"
   matwrite(string(dirname)"/linear_approximations_real"string(gen_inflation)"_line_"string(linenum)"_algorithm_"string(algorithm)".mat",Dict("coeff_const"=>coeff_const,"coeff_p"=>coeff_p,"coeff_q"=>coeff_q,"approx_error"=>approx_error))
elseif quantity_to_approx == "line_reactive_power"
   matwrite(string(dirname)"/linear_approximations_reactive"string(gen_inflation)"_line_"string(linenum)"_algorithm_"string(algorithm)".mat",Dict("coeff_const"=>coeff_const,"coeff_p"=>coeff_p,"coeff_q"=>coeff_q,"approx_error"=>approx_error))
