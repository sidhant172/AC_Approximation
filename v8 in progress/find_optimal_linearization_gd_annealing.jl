...
function find_optimal_linearizations(network_data, to_approx_list, inflation_factors, solver, solver_lp, cnst_gen_max_iter, tol, obj_tuning)
    ...
    for (i,to_approx) in to_approx_list
        approximation = find_optimal_linearization(network_data, pg_init, qg_init, to_approx, solver, solver_lp, cnst_gen_max_iter, tol, obj_tuning)
...
function find_optimal_linearization(network_data, pg_init, qg_init, to_approx, solver, solver_lp, cnst_gen_max_iter, tol, obj_tuning)
...
while (current_inflation < network_data["gen_inflation"])

@show network_data["gen_inflation"]
@show current_inflation = current_inflation + 0.03

current_inflation = min(current_inflation,network_data["gen_inflation"])
...
network_data["load_inflation"] = current_inflation



# initialize parameters for the for loop
step_factor = 1
warm = true
backtrack = true
ctr = 0
