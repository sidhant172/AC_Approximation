...
function post_soc_opf_maxerror(data::Dict{String,Any}, model, aux_data)
    ...
    for (i,bus) in ref[:bus]
        ...
        if i in aux_data["active_buses"]
            @constraint(model,
                sum(p[a] for a in ref[:bus_arcs][i]) +
                sum(p_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) == pb[i] - sum(shunt["gs"] for shunt in bus_shunts)*w[i]
            )

            @constraint(model,
                sum(q[a] for a in ref[:bus_arcs][i]) +
                sum(q_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) == qb[i] + sum(shunt["bs"] for shunt in bus_shunts)*w[i]
            )
        else
            @constraint(model,
                sum(p[a] for a in ref[:bus_arcs][i]) +
                sum(p_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) == -sum(shunt["gs"] for shunt in bus_shunts)*w[i]
            )

            @constraint(model,
                sum(q[a] for a in ref[:bus_arcs][i]) +
                sum(q_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) == sum(shunt["bs"] for shunt in bus_shunts)*w[i]
            )
        end
    end
