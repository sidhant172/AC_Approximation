...
################################################################################
    ...
    for iter=1:cnst_gen_max_iter   # total iterations of constraint generation
        ...
        # model, var_refs = post_qc_opf_maxerror(network_data, model, aux_data)
...
function find_optimal_linearization_gradient_descent(network_data, aux_data, jacobian_filename, solver, solver_lp, max_iter, tol, step_size)
    ...
    for iter = 1:max_iter
        ...
        # regularized ac opf
        initial_point = Dict{String,Any}()
        initial_point["pb"] = current_sol["pb"]
        initial_point["qb"] = current_sol["qb"]
        model = JuMP.Model(solver = solver)
        model, var_refs = post_ac_opf_projection(network_data, model, aux_data, initial_point)
        # model, var_refs = post_qc_opf_maxerror(network_data, model, aux_data)
        status = solve(model)
        current_sol = get_current_solution(network_data,model,var_refs,aux_data)
        # end of regularized ac opf
