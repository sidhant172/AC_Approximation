...
function post_ac_opf_projection(data::Dict{String,Any}, model, aux_data, initial_point)
    @assert !haskey(data, "multinetwork")
    @assert !haskey(data, "conductors")
    ...
    if aux_data["quantity"] == "line_real_power"
        @NLobjective(model, Max, aux_data["obj_tuning"]*(aux_data["sign"]*p[aux_data["quantity_index"]] - aux_data["sign"]*(aux_data["l0"] +
            sum(aux_data["l_pb"][i]*pb[i] + aux_data["l_qb"][i]*qb[i] for i in aux_data["active_buses"]) ))
            + 0.1*sum((initial_point["pb"][i] - pb[i])^2 +  (initial_point["qb"][i] - qb[i])^2  for i in aux_data["active_buses"]))
    elseif aux_data["quantity"] == "line_reactive_power"
        @NLobjective(model, Max, aux_data["obj_tuning"]*(aux_data["sign"]*q[aux_data["quantity_index"]] - aux_data["sign"]*(aux_data["l0"] +
            sum(aux_data["l_pb"][i]*pb[i] + aux_data["l_qb"][i]*qb[i] for i in aux_data["active_buses"]) ))
            + 0.1*sum((initial_point["pb"][i] - pb[i])^2 +  (initial_point["qb"][i] - qb[i])^2  for i in aux_data["active_buses"]))
    ...
    # @objective(model, Min, sum((initial_point["pb"][i] - pb[i])^2 +  (initial_point["qb"][i] - qb[i])^2  for i in aux_data["active_buses"]))
    ...
    for (i,bus) in ref[:bus]
        ...
        if i in aux_data["active_buses"]
            ...
            @constraint(model,
                ...
                sum(p_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) ==
                pb[i] -
                sum(shunt["gs"] for shunt in bus_shunts)*w[i]^2
            )
            @constraint(model,
                sum(q[a] for a in ref[:bus_arcs][i]) +
                sum(q_dc[a_dc] for a_dc in ref[:bus_arcs_dc][i]) ==
                qb[i] +
                sum(shunt["bs"] for shunt in bus_shunts)*w[i]^2
            )
        else
            # Bus KCL
            @constraint(model,
...
"""
function post_qc_opf_maxerror(data::Dict{String,Any}, model, aux_data)
    @assert !haskey(data, "multinetwork")
    @assert !haskey(data, "conductors")
