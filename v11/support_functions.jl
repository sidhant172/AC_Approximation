...
function create_aux_data(network_data,inflation_factors, solver, obj_tuning)
    ...
    load_buses = [load["load_bus"] for (i,load) in network_data["load"]]
    ...
    for i in load_buses
        ...
        pdmin[i] = min((1-load_inflation)*sum(load["pd"] for load in bus_loads),(1+load_inflation)*sum(load["pd"] for load in bus_loads))
        pdmax[i] = max((1-load_inflation)*sum(load["pd"] for load in bus_loads),(1+load_inflation)*sum(load["pd"] for load in bus_loads))
        qdmin[i] = min((1-load_inflation)*sum(load["qd"] for load in bus_loads),(1+load_inflation)*sum(load["qd"] for load in bus_loads))
        qdmax[i] = max((1-load_inflation)*sum(load["qd"] for load in bus_loads),(1+load_inflation)*sum(load["qd"] for load in bus_loads))
